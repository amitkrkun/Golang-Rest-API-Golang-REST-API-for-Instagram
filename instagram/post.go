...
import (
	...
	uploadPost "nimeshjohari02.com/restapi/filehandler"
	getData "nimeshjohari02.com/restapi/user"
...
func AddPostUser(userID string, fileName string) (mongo.UpdateResult, error) {
	...
	collection := conn.Database("rest").Collection("Users")
...
func AddInstaPost(w http.ResponseWriter, r *http.Request) {
	if (r.Method == "POST") {
		...
		user := getData.FetchUser(post.userID)
